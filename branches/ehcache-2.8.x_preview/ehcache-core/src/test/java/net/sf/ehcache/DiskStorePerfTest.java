...
public class DiskStorePerfTest {
    ...
    public void testOverflowToDiskWithLargeNumberofCacheEntriesAndGets() throws Exception {
        CacheManager manager = new CacheManager(new Configuration().name("testOverflowToDiskWithLargeNumberofCacheEntriesAndGets")
                  .diskStore(new DiskStoreConfiguration().path(diskFolder.getRoot().getAbsolutePath())));
      try {
    ...
    public void testMaximumCacheEntriesIn64MBWithOverflowToDisk() throws Exception {
        CacheManager manager = new CacheManager(new Configuration().name("testMaximumCacheEntriesIn64MBWithOverflowToDisk")
                .diskStore(new DiskStoreConfiguration().path(diskFolder.getRoot().getAbsolutePath())));
        try {
            Cache cache = new Cache("test", 1000, MemoryStoreEvictionPolicy.LRU, true, null, true, 500, 500, false, 1, null);
            manager.addCache(cache);
    ...
    public void testLargePutGetPerformanceWithOverflowToDisk() throws Exception {
        CacheManager manager = new CacheManager(new Configuration().name("testLargePutGetPerformanceWithOverflowToDisk")
                .diskStore(new DiskStoreConfiguration().path(diskFolder.getRoot().getAbsolutePath())));
        try {
