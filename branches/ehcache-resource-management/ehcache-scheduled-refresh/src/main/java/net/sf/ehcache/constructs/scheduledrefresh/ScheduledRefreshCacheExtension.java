...
public class ScheduledRefreshCacheExtension implements CacheExtension {
    ...
    /*
    This is more complicated at first blush than you might expect, but it attempts to prove that
    one and only one trigger is scheduled for this job, and that it is the right one. Even if multiple cache
    extensions for the same cache are sharing the Quartz store, it should end up with only one trigger winning. If
    there are multiple *different* cron expressions, someone will win, but it is not deterministic as to which one.
     */
    private void scheduleOverseerJob(JobDetail seed) throws SchedulerException {
        ...
        // delete existing triggers. Remember the job is durable, so no problem here.
        ...
        // now, keep trying to add and count the trigger, until there is only 1 trigger there.
        boolean done = false;
        int retryCount = 0;
        do {
            // schedule ours
     ...
     * other nodes (in a clustered environment) from running. Until and unless the
     * last scheduled refresh cache extension for a particular cache/cache manager/unique name
     * is shutdown, they will continue to run. This is only an issue for clustered,
     * TerracottaJobStore-backed scheduled refresh cache extensions.
     *
