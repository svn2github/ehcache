        ...
        try {
            ...
            if (oldElement != null && oldElement.getObjectValue() instanceof SoftLock) {
                ...
                if (!oldSoftLock.getTransactionID().equals(getCurrentTransactionContext().getTransactionId())) {
                    ...
                    LOG.debug("put: cache [{}] key [{}] soft locked in current transaction", cacheName, key);
        ...
        try {
            ...
            if (element.getObjectValue() instanceof SoftLock) {
                ...
                if (softLock.getTransactionID().equals(getCurrentTransactionContext().getTransactionId())) {
                    LOG.debug("get: cache [{}] key [{}] soft locked in current transaction", cacheName, key);
                    return softLock.getNewElement();
                } else {
                    LOG.debug("get: cache [{}] key [{}] soft locked in different transaction", cacheName, key);
    ...
    public Element getQuiet(Object key) {
        ...
        try {
            ...
            if (element.getObjectValue() instanceof SoftLock) {
                SoftLock softLock = (SoftLock) element.getObjectValue();
                if (softLock.getTransactionID().equals(getCurrentTransactionContext().getTransactionId())) {
                    LOG.debug("getQuiet: cache [{}] key [{}] soft locked in current transaction", cacheName, key);
                    return softLock.getNewElement();
                } else {
                    LOG.debug("getQuiet: cache [{}] key [{}] soft locked in different transaction", cacheName, key);
                    return softLock.getOldElement();
                }
            } else {
        ...
        try {
            ...
            if (oldElement.getObjectValue() instanceof SoftLock) {
                ...
                if (!oldSoftLock.getTransactionID().equals(getCurrentTransactionContext().getTransactionId())) {
                    ...
                    LOG.debug("remove: cache [{}] key [{}] soft locked in current transaction", cacheName, key);
                    Element previousElement = oldSoftLock.getNewElement();
