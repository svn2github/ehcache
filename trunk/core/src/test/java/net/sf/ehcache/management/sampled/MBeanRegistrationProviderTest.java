...
public class MBeanRegistrationProviderTest extends AbstractCacheTest {
    ...
    private static final Logger LOG = Logger
            .getLogger(MBeanRegistrationProviderTest.class.getName());
    ...
    private void assertSampledMBeansGroupRegistered(int size) throws Exception {
        Set queryNames = mbeanServer.queryNames(new ObjectName(
                SampledEhcacheMBeans.GROUP_ID + ":*"), null);
        assertEquals(size, queryNames.size());
    }

    private void assertCacheManagerMBeansRegistered(String cacheManagerName,
            int size) throws Exception {
        Set queryNames = mbeanServer.queryNames(SampledEhcacheMBeans
                .getCacheManagerObjectName(cacheManagerName), null);
        assertEquals(size, queryNames.size());
    }
    ...
    public void doTestMonitoringAutodetect(boolean dsoActive) throws Exception {
        File file = new File(TEST_CONFIG_DIR
                + "ehcache-monitoring-autodetect.xml");
        Configuration configuration = ConfigurationFactory
                .parseConfiguration(file);
        cacheManager = new CacheManager(configuration);
        if (dsoActive) {
