...
public abstract class CachingFilter extends Filter {
	 ...
	 */
	protected Ehcache filterCache;

	/**
	 * Initialises filterCache to use.
	 * <p/>
	 * 
	...
	protected void doFilter(final HttpServletRequest request,
			...
			FilterNonReentrantException, Exception {
	...
	protected PageInfo buildPageInfo(final HttpServletRequest request,
		...
		Element element = filterCache.get(key);
		if (element == null || element.getObjectValue() == null) {
			// Page is not cached - build the response, cache it, and
			// send to client
			pageInfo = buildPage(request, response, chain);
			if (pageInfo.isOk()) {
				Element prevElement = filterCache.putIfAbsent(new Element(key, pageInfo));
				if (prevElement == null) { // put succeeded
					if (LOG.isDebugEnabled()) {
						LOG.debug("PageInfo ok. Adding to cache "
								+ filterCache.getName() + " with key " + key);
					}
				}
			} else {
				if (LOG.isDebugEnabled()) {
				...
				filterCache.putIfAbsent(new Element(key, null));
			}
		} else {
	...
	protected PageInfo buildPage(final HttpServletRequest request,
		...
		long timeToLiveSeconds = filterCache.getCacheConfiguration()
