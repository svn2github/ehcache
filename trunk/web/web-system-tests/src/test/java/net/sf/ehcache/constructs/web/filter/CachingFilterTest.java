...
public class CachingFilterTest extends AbstractWebTest {
    ...
    public void testContentTypeByContentType() throws Exception {
        ...
        String contentType = httpMethod.getResponseHeader("Content-Type").getValue().replaceAll(" ", "");
        assertNull(responseBody);
        assertContains("text/html;charset=utf-8", contentType.toLowerCase());
    }
    ...
    public void testSetThenAddHeaders() throws Exception {
        String url = buildUrl("/headers/SetThenAddHeaders.jsp");
        HttpClient httpClient = new HttpClient();
        HttpMethod httpMethod = new GetMethod(url);
        int responseCode = httpClient.executeMethod(httpMethod);
        assertEquals(HttpURLConnection.HTTP_OK, responseCode);
    ...
    public void testAddThenSetHeaders() throws Exception {
        String url = buildUrl("/headers/AddThenSetHeaders.jsp");
        HttpClient httpClient = new HttpClient();
        HttpMethod httpMethod = new GetMethod(url);
        int responseCode = httpClient.executeMethod(httpMethod);
        assertEquals(HttpURLConnection.HTTP_OK, responseCode);
