...
public class ExplicitLockingCache implements Ehcache {
	...
	public ExplicitLockingCache(Ehcache cache) throws CacheException {
		...
		if (cache.getCacheConfiguration().isTerracottaClustered()) {
			this.cacheLockProvider = ((CacheLockProvider) cache
					.getInternalContext());
		} else {
			this.cacheLockProvider = new StripedReadWriteLockSync(
					StripedReadWriteLockSync.DEFAULT_NUMBER_OF_MUTEXES);
		}
	 ...
	 * <ol>
	 * <li>the idle time is non-zero and has elapsed, unless the cache is
	 * eternal; or
	 * <li>the time to live is non-zero and has elapsed, unless the cache is
	 * eternal; or
	...
	/**
	 * Clones a cache. This is only legal if the cache has not been initialized.
	 * At that point only primitives have been set and no MemoryStore or
	 * DiskStore has been created.
	 * <p/>
	...
	/**
	 * Use this to access the service in order to register and unregister
	 * listeners
	 * 
	...
	/**
	 * Whether an Element is stored in the cache in Memory, indicating a very
	...
	/**
	 * Whether an Element is stored in the cache on Disk, indicating a higher
	 * cost of retrieval.
	 * 
	...
	/**
	 * The GUID for this cache instance can be used to determine whether two
	 * cache instance references are pointing to the same cache.
	 * 
	 * @return the globally unique identifier for this cache instance. This is
	...
	/**
	 * Gets the CacheManager managing this cache. For a newly created cache this
	 * will be null until it has been added to a CacheManager.
	 * 
	...
	/**
	 * Causes all elements stored in the Cache to be synchronously checked for
	 ...
	 * @return true if an Element matching the key is found in the cache. No
	 *         assertions are made about the state of the Element.
	 */
	...
	/**
	 * Gets an immutable Statistics object representing the Cache statistics at
	 * the time. How the statistics are calculated depends on the statistics
	 * accuracy setting. The only aspect of statistics sensitive to the accuracy
	 * setting is object size. How that is calculated is discussed below. <h3>
	 * Best Effort Size</h3> This result is returned when the statistics
	 * accuracy setting is
	 * {@link net.sf.ehcache.Statistics#STATISTICS_ACCURACY_BEST_EFFORT}.
	 * <p/>
	 * The size is the number of {@link net.sf.ehcache.Element}s in the
	 * {@link net.sf.ehcache.store.MemoryStore} plus the number of
	 * {@link net.sf.ehcache.Element}s in the
	 ...
	 * This result is returned when the statistics accuracy setting is
	 * {@link net.sf.ehcache.Statistics#STATISTICS_ACCURACY_GUARANTEED}.
	 * <p/>
	...
	/**
	 * Accessor for the BootstrapCacheLoader associated with this cache. For
	 * testing purposes.
	 */
	...
	/**
	 * DiskStore paths can conflict between CacheManager instances. This method
	 * allows the path to be changed.
	 * 
	 * @param diskStorePath
	 ...
	 * {@link net.sf.ehcache.store.MemoryStore} or a
	 * {@link net.sf.ehcache.store.DiskStore}.
	 * <p/>
	...
	/**
	 * Bootstrap command. This must be called after the Cache is intialised,
	 * during CacheManager initialisation. If loads are synchronous, they will
	 * complete before the CacheManager initialise completes, otherwise they
	 ...
	 * {@link #put} is done to put an Element in.
	 * <p/>
	 ...
	 * See {@link net.sf.ehcache.constructs.blocking.SelfPopulatingCache} for an
	 * example.
	 * <p/>
	 * Note. If a LockTimeoutException is thrown while doing a <code>get</code>
	 * it means the lock was never acquired, therefore it is a threading error
	 * to call {@link #put}
	 * 
	 * @throws LockTimeoutException
	 *             if timeout millis is non zero and this method has been unable
	 *             to acquire a lock in that time
	 * @throws RuntimeException
	 *             if thrown the lock will not released. Catch and call
	 *             <code>put(new Element(key, null));</code> to release the lock
	 *             acquired.
	 */
	 ...
	 * <li>if the element exists in the cache, that an update has occurred, even
	 * if the element would be expired if it was requested
	 * </ul>
	 * 
	 * @param element
	 *            An object. If Serializable it can fully participate in
	 *            replication and the DiskStore.
	 * @param doNotNotifyCacheReplicators
	 *            whether the put is coming from a doNotNotifyCacheReplicators
	 *            cache peer, in which case this put should not initiate a
	 *            further notification to doNotNotifyCacheReplicators cache
	 *            peers
	 * @throws IllegalStateException
	 *             if the cache is not
	 *             {@link net.sf.ehcache.Status#STATUS_ALIVE}
	...
	/**
	 * Put an element in the cache, without updating statistics, or updating
	 * listeners. This is meant to be used in conjunction with {@link #getQuiet}
	 * 
	 * @param element
	 *            An object. If Serializable it can fully participate in
	 *            replication and the DiskStore.
	 ...
	 * Use {@link #getQuiet(Object)} to peak into the Element to see its last
	 * access time with get
	 * 
	 * @param key
	 ...
	 * 
	 * @return a list of {@link Object} keys for this cache. This is not a live
	 *         set, so it will not track changes to the key set.
	 */
	 ...
	 * <p/>
	 * The time taken is O(n), where n is the number of elements in the cache.
	 * On a 1.8Ghz P4, the time taken is approximately 200ms per 1000 entries.
	 * This method is not synchronized, because it relies on a non-live list
	 * returned from {@link #getKeys()} , which is synchronised, and which takes
	 * 8ms per 1000 entries. This way cache liveness is preserved, even if this
	 * method is very slow to return.
	 * <p/>
	...
	/**
	 * Returns a list of all elements in the cache, whether or not they are
	 * expired.
	 * <p/>
	 * The returned keys are not unique and may contain duplicates. If the cache
	 * is only using the memory store, the list will be unique. If the disk
	 * store is being used as well, it will likely contain duplicates, because
	 * of the internal store design.
	 * <p/>
	 * The List returned is not live. It is a copy.
	 * <p/>
	 * The time taken is O(log n). On a single cpu 1.8Ghz P4, approximately 6ms
	 * is required for 1000 entries and 36 for 50000.
	 * <p/>
	...
	/**
	 * Removes an {@link net.sf.ehcache.Element} from the Cache. This also
	 * removes it from any stores it may be in.
	 * <p/>
	 * Also notifies the CacheEventListener after the element was removed, but
	 * only if an Element with the key actually existed.
	 * 
	 * @param key
	 *            the key to remove
	 * @return true if the element was removed, false if it was not found in the
	 *         cache
	 * @throws IllegalStateException
	 *             if the cache is not
	 *             {@link net.sf.ehcache.Status#STATUS_ALIVE}
	...
	/**
	 * Removes an {@link net.sf.ehcache.Element} from the Cache, without
	 * notifying listeners. This also removes it from any stores it may be in.
	 * <p/>
	 * 
	 * @param key
	 *            the key to remove
	 ...
	 * <p/>
	 * The time taken for this method to return is a useful measure of runtime
	...
	/**
	 * Register a {@link net.sf.ehcache.extension.CacheExtension} with the
	 * cache. It will then be tied into the cache lifecycle.
	 */
	...
	/**
	 * Unregister a {@link net.sf.ehcache.extension.CacheExtension} with the
	 * cache. It will then be detached from the cache lifecycle.
	 */
	...
	/**
	 * Sets an ExceptionHandler on the Cache. If one is already set, it is
	 * overwritten.
	 */
	...
	/**
	 * Register a {@link CacheLoader} with the cache. It will then be tied into
	 * the cache lifecycle.
	 * <p/>
	...
	public void registerCacheLoader(CacheLoader cacheLoader) {
		throw new CacheException(
				"This method is not appropriate for a blocking cache.");
	}
	 ...
	 * {@link #setDisabled(boolean)} method.
	 * <p/>
	 * By default caches are enabled on creation, unless the
	...
	/**
	 * Disables or enables this cache. This call overrides the previous value of
	 * disabled, even if the <code>net.sf.ehcache.disabled</code> system
	 * property is set
	 * <p/>
	 * 
	 * @param disabled
	...
	/**
	 * Put an element in the cache if an element for the key doesn't already
	 * exist
	 * 
	 * @param element
	 *            to add to the cache
	 ...
	 */
	public boolean putIfAbsent(Element e) {
		try {
			acquireWriteLockOnKey(e.getKey());
			if (!isKeyInCache(e.getKey())) {
				this.put(e);
				return true;
			}
			return false;
		} finally {
			releaseWriteLockOnKey(e.getKey());
		}
