...
public abstract class ClientBase extends AbstractClientBase {
	...
	protected synchronized final ToolkitBarrier getBarrierForAllClients() {
		if (barrier == null) {
			barrier = getClusteringToolkit().getBarrier(
					"barrier with all clients", getParticipantCount());
		}
		return barrier;
	}

	protected final int waitForAllClients() throws InterruptedException,
	...
	protected void setupCacheManager() {
		cacheManager = new CacheManager(
				Client1.class.getResourceAsStream("/ehcache-config.xml"));
	}
	...
	private Toolkit createToolkit() {
		try {
			return ToolkitFactory.createToolkit("toolkit:terracotta://"
					+ getTerracottaUrl());
	...
	protected abstract void runTest(Cache cache, Toolkit myToolkit)
			throws Throwable;

	public void waitForAllCurrentTransactionsToComplete(Cache cache) {
		// Only do waitFor All Txn for Clustered Caches
		if (cache.getCacheConfiguration().isTerracottaClustered()) {
			Toolkit internalToolkit = ToolkitClientAccessor
					.getInternalToolkitClient(cache);
	...
	public void waitForAllCurrentTransactionsToComplete(Toolkit toolkitParam) {
		try {
			Method method = Toolkit.class.getDeclaredMethod(
					"waitUntilAllTransactionsComplete", (Class<?>[]) null);
			method.invoke(toolkitParam, (Object[]) null);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
