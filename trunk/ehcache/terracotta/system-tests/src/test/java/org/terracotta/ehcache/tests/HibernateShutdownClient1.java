...
public class HibernateShutdownClient1 extends ClientBase {
  ...
  public static void main(String[] args) throws Exception {
    HibernateShutdownClient1 client = new HibernateShutdownClient1(args);
    client.doTest();
  }
  ...
  public void testClusteredCache() {
    try {
      ...
      barrier.await(TimeUnit.SECONDS.toMillis(3 * 60)); // wait for client2 to assert clustered cache
      barrier.await(TimeUnit.SECONDS.toMillis(3 * 60)); // line up for client2 to wait for client1 shutdown
    } catch (Throwable e) {
      e.printStackTrace();
    }
  }
  ...
  private Set<SimpleThreadInfo> filterKnownThreads(Set<SimpleThreadInfo> dump) {
    List<ThreadIgnore> ignores = Arrays.asList(
        new ThreadIgnore("http-", "org.apache.tomcat."),
  ...
  public void storeL1ClassLoaderWeakReferences() throws Exception {
    Barrier barrier = getClusteringToolkit().getBarrier("shutdownBarrier", 2);
    ClassLoader clusteredStateLoader = barrier.getClass().getClassLoader();
