...
public class LoaderClient {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String specialClasspath = args[1];
    ...
    for (int i = 0; i < urls.length; i++) {
      ...
      if (!specialCpEntries[i].endsWith(".jar") && !specialCpEntries[i].endsWith("/"))
        specialCpEntries[i] = specialCpEntries[i] + "/";

      if (specialCpEntries[i].endsWith(".jar"))
        urls[i] = sneakDotDotInUrl(new URL(specialCpEntries[i]));
      else
        urls[i] = new URL(specialCpEntries[i]);
    }
  ...
  private static URL sneakDotDotInUrl(URL url) throws MalformedURLException {
    ...
    if (pathEntries.size() < INDEX_WHERE_TO_SNEAK_DOT_DOT + 1) {
      throw new AssertionError("path to url (" + url + ") is too short - it must contain at least " +
              (INDEX_WHERE_TO_SNEAK_DOT_DOT + 1) + " path separators");
    }
  ...
  private static byte[] getClassBytes(Class<?> clazz) throws IOException {
    ...
    InputStream in = LoaderClient.class.getClassLoader().
  ...
  public static class Asserter implements Runnable {
    ...
    public void run() {
      ...
      if (!cache.get("1").getValue().equals("one"))
